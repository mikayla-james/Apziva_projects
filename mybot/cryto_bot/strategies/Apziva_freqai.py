...
class ApzivaAI(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["ST_long"] = pta.supertrend(dataframe["high"], dataframe["low"], dataframe["close"], 
                                              length=period, multiplier=atr_multiplier)[f"SUPERTl_{period}_{atr_multiplier}"]
        dataframe["ST_short"] = pta.supertrend(dataframe["high"], dataframe["low"], dataframe["close"], 
                                              length=period, multiplier=atr_multiplier)[f"SUPERTs_{period}_{atr_multiplier}"]
        dataframe["rsi"] = ta.RSI(dataframe, timeperiod=14)
        dataframe["mfi"] = ta.MFI(dataframe, timeperiod=14)
        bollinger = qtpylib.bollinger_bands(qtpylib.typical_price(dataframe), window=20, stds=2.0)
        dataframe["bb_width"] = bollinger["upper"] - bollinger["lower"]
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        if enter_long_conditions:
            df.loc[
                reduce(lambda x, y: x & y, enter_long_conditions), "enter_long"
            ] = 1
